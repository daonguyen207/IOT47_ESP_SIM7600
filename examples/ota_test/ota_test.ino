...
/*  Note
 *   Đây là chương trình ota thông qua ftp server. Bạn phải có 1 ftp server để lưu trữ file .bin ( có thể dùng 000webhost)
 *   Điều nàu đồng nghĩa với việc bạn phải phân vùng cho spiffs đủ bộ nhớ để chứa file .bin
 *   Sử dung ftp_at.h để download file .bin lưu vào phân vùng snpiff của esp rồi tiến hành update bằng hàm performUpdate
 *   
 *   
 ...
 *  5. Sau khi download file thành công (mã lỗi 0) thì gọi performUpdate để tiến hành cập nhật firmware mới
...
void SIM_on_lte(void *arg)
{
  ...
  if(err == 0) //tải xuống thành công
  {
    File file = LittleFS.open(FTP_FILE_IN_ESP,"r");
    if(file)performUpdate(file, file.size()); //tiền hành cập nhật vào phần vùng code chính
  }
}
