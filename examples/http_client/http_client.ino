 ...
 *  2.Dùng https_get https_put https_head https_delete để gọi api, lưu ý đây là hàm bất đồng bộ, nó sẽ bỏ qua lần gọi hàm tiếp theo nếu đang bận
...
void loop() 
{
  ...
  if(LTE_OK)
  {
    if(millis() - tSent > 3000) //3s call 1 lần
    {
      tSent = millis();
      https_get("https://i47tserver.top/time.php",[](int http_code,uint8_t *payload, int len){
        Serial.println("\r\nHTTP Code:" + String(http_code) + " Payload length:" + String(len));
        if(http_code>0)for(int i=0;i<len;i++)Serial.write(payload[i]);
        else
        {
          Serial.println("Request lỗi, vui lòng thử lại");
        }
      });
    }
  }
